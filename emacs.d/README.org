...
#+begin_src emacs-lisp
  ...
  (use-package ido-vertical-mode
    :config
    (ido-vertical-mode 1)
    (setq ido-vertical-define-keys 'C-n-C-p-up-and-down-only
...
Make sure we store recent files. This lets ido do fancy things.
...
#+begin_src emacs-lisp
  (use-package evil
    ...
    (setq evil-emacs-state-cursor    (list (belak/zenburn-color "red")    'box)
          ...
          evil-operator-state-cursor (list (belak/zenburn-color "cyan")    'evil-half-cursor))
...
- [ ] Make =subword-mode= diminish work correctly
